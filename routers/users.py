...
from dependencies import get_supa_db, oauth2_scheme
...
def get_all(db: Session = Depends(get_supa_db), token: str = Depends(oauth2_scheme)):
...
def get_my_profile(db: Session = Depends(get_supa_db), token: str = Depends(oauth2_scheme)):
    token_data = credentials(token)
    return get_current_user(db, token_data.email)

@router.get("/{id}", tags=["User"], summary="Get One User")
def get(id : str = Path, db: Session = Depends(get_supa_db), token: str = Depends(oauth2_scheme)):
    token_data = credentials(token)
    return get_user(id, db)
   
    
@router.delete("/{id}", tags=["User"], summary="Delete One User")
def delete(id: str = Path, db : Session = Depends(get_supa_db), token: str = Depends(oauth2_scheme)):
    token_data = credentials(token)
